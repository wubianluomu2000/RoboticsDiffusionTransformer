...
from utils import save_images, save_videos, save_results, interactive
...
def parse_args(args=None):
    ...
    parser.add_argument("--pretrained_path", type=str, default="/data/jasonyao/rdt/maniskill-model/mp_rank_00_model_states.pt", help="Path to the pretrained model")
...
# check task
args = parse_args()
if args.env_id not in task2lang.keys():
    env_ids = interactive(task2lang)
else:
    env_ids = [args.env_id]
...
model_name = os.path.split(args.pretrained_path)[1]
...
for env_id in env_ids:
    print('#'*100)
    print(f'task {env_id} has started...')
    env = gym.make(
        env_id,
    ...
    if os.path.exists(f'lang_embeds/text_embed_{env_id}.pt'):
        text_embed = torch.load(f'lang_embeds/text_embed_{env_id}.pt')
    else:
            ...
            for idx in range(actions.shape[0]):
                ...
                if terminated or truncated:
                    ...
                    if info['success']:
                        ...
                        break 
        print(f"Trial {episode} finished, success: {info['success']}, steps: {global_steps}")
        eval_result.append(f"Trial {episode} finished, success: {info['success']}, steps: {global_steps}\r\n")

        video_name = f'videos/RDT/{model_name}/{env_id}/traj_{episode:02}.mp4'
        save_videos(video_frames, video_name)

        output_path = f'imgs/RDT/{model_name}/{env_id}/traj_{episode:02}'
        save_images(video_frames, output_path)
    ...
    # Save eval result
    eval_result.append('\r\n' + '#'*200 + '\r\n')
    eval_result.append(f"env_id: RDT-{model_name}   {env_id}\r\nSuccess rate: {success_rate}%\r\n")
    result_path = f'eval_result/RDT/{env_id}.txt'
    save_results(eval_result, result_path)
